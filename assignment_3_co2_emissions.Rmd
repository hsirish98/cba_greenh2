...
no_add_scc <- no_additionality_co2 %>%
  filter(year >= "2024") %>%
   mutate("counterfactual_emissions" = counterfactual$total) %>%
  mutate(
    total_reduction = counterfactual_emissions- total,
    t = seq(1, 27), 
...
ggplot() +
  ...
  labs(x = "Discount rate, r", y = "NPV ($M)", title = "Partial Sensitivity Analysis Additioality Scenario") +
  theme_minimal() +
